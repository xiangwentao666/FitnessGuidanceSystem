...
class ProcessMasks():
    ...
    def __call__(self, pipe, plot_heart_rate_sender_pipe, source):
        self.mask_process_receiver_pipe = pipe
        self.plot_heart_rate_sender_pipe = plot_heart_rate_sender_pipe
        self.source = source
    ...
    def rec_frames(self):
        while True and not self.stop:
            data = self.mask_process_receiver_pipe.recv()
    ...
    def process_signal(self, batch_mean):
        ...
        self.hrs.append(hr)
        if self.plot_heart_rate_sender_pipe is not None and self.stop:
            self.plot_heart_rate_sender_pipe.send(None)
        elif self.plot_heart_rate_sender_pipe is not None:
            self.plot_heart_rate_sender_pipe.send([p, self.hrs])
        else:
    ...
    def extract_signal(self):
        ...
        while True and not self.stop:
            ...
            if mean_dict['face_detected'] == False:
                if self.plot_heart_rate_sender_pipe is not None:
                    self.plot_heart_rate_sender_pipe.send('no face detected')
                continue
